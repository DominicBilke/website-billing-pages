...
$page_title = 'Edit Work Billing Entry';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $work_name = sanitize_input($_POST['work_name']);
    ...
    $stmt = mysqli_prepare($conn, "UPDATE work_billing SET work_name=?, project=?, date=?, hours=?, rate=?, total=?, description=? WHERE id=?");
    mysqli_stmt_bind_param($stmt, "sssdddssi", $work_name, $project, $date, $hours, $rate, $total, $description, $id);
    
    if (mysqli_stmt_execute($stmt)) {
        $content = display_message('Entry updated successfully.', 'success');
        header("refresh:2;url=work_overview.php");
    } else {
...
// Get current entry
$stmt = mysqli_prepare($conn, "SELECT * FROM work_billing WHERE id = ?");
...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    <div class="card">
        <div class="card-header">
            <h4>Edit Work Billing Entry</h4>
        </div>
        <div class="card-body">
            <form method="POST" action="">
                <div class="mb-3">
                    <label for="work_name" class="form-label">Work Name</label>
                    <input type="text" class="form-control" id="work_name" name="work_name" value="<?php echo htmlspecialchars($row['work_name']); ?>" required>
                </div>
                <div class="mb-3">
                ...
                <a href="work_overview.php" class="btn btn-secondary">Cancel</a>
            </form>
        </div>
    </div>
