...
$pdo = new PDO('mysql:host=localhost;dbname='.$_SESSION['dbname'], $_SESSION['dbuser'], $_SESSION['dbpsw']);
       ...
       if($_SESSION['firmen_status'] == 'firma' )
 	 { 
...
$tlohn = number_format($tlohn, 2, ",", ".");
    ...
    <h3 class="h3"><a href="script_firma/pdf_erstellen_einzeln.php'.$inc_string.'" class="text-info">[DOWNLOAD ALS PDF]</a></h3>
    <p>'.$name.'<br/>
    '.$str_nr.'<br/>
    '.$plz_ort.'</p>
...
echo '				    </select>
    ...
    <p>Dauer / Tag: '.$dauer_h.' Stunden '.$dauer_min.' Minuten</p>
    <p>Pause / Tag: '.$pause_h.' Stunden '.$pause_min.' Minuten</p>
    <p>Arbeitszeit / Tag: '.$zeit_h.' Stunden '.$zeit_min.' Minuten</p>
    <p>Arbeitsstunden / Tag: '.$total.' Stunden</p>'; 
}
?>