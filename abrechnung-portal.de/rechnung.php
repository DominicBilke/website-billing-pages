...
$rechnungs_nummer = $_GET['Nr'];
$rechnungs_datum = date("d.m.Y");
$lieferdatum = $_GET['Lieferdatum'];
$leistungszeitraum = $_GET['Leistungszeitraum'];
$pdfAuthor = "Abrechnung-Portal";

$rechnungs_header = $_GET['Firma'].'
'.$_GET['Name'].'
'.$_GET['Str_Nr'].'
'.$_GET['PLZ_Ort'];

$rechnungs_empfaenger = '<span style="text-decoration: underline; font-size: 6pt">'.$_GET['Firma']." | ".$_GET['Str_Nr']." | ".$_GET['PLZ_Ort']."<br><br></span>\n".$_GET['Empfaenger'];

$rechnungs_footer = "Wir bitten um eine Begleichung der Rechnung innerhalb von ".$_GET['Ziel']." Tagen nach Erhalt.<br/>Bitte Überweisen Sie den vollständigen Betrag an:

<b>Empfänger:</b> ".$_GET['Kontoinhaber']."
<b>IBAN</b>: ".$_GET['IBAN']."
<b>BIC</b>: ".$_GET['BIC'];
...
$rechnungs_posten = array(
	array($_GET['Abrechnung'], $_GET['Anzahl'], $_GET['Lohn'] ));

//Höhe eurer Umsatzsteuer. 0.19 für 19% Umsatzsteuer
$umsatzsteuer = $_GET['Steuer']; 
$steuernummer = $_GET['Steuer-Nr']; 
...
Lieferdatum: '.$lieferdatum.'<br>'.($leistungszeitraum ? 'Leistungszeitraum: '.$leistungszeitraum.'<br/>' : '').'
