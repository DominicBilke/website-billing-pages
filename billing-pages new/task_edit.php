...
$page_title = 'Edit Task Billing Entry';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE task_billing SET task_name=?, project=?, date=?, hours=?, rate=?, total=?, description=? WHERE id=?");
    mysqli_stmt_bind_param($stmt, "sssdddssi", $task_name, $project, $date, $hours, $rate, $total, $description, $id);
...
// Get current entry
$stmt = mysqli_prepare($conn, "SELECT * FROM task_billing WHERE id = ?");
...
if ($row = mysqli_fetch_assoc($result)) {
    ...
    <div class="card">
        <div class="card-header">
            <h4>Edit Task Billing Entry</h4>
        </div>
        <div class="card-body">
            <form method="POST" action="">
                <div class="mb-3">
                    <label for="task_name" class="form-label">Task Name</label>
                    <input type="text" class="form-control" id="task_name" name="task_name" value="<?php echo htmlspecialchars($row['task_name']); ?>" required>
                </div>
                <div class="mb-3">
